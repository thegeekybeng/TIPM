...
export const CountryComparisonChart: React.FC<CountryComparisonProps> = ({ countries, onCountrySelect }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <h3 className="text-xl font-bold text-gray-800 mb-4">Country Tariff Comparison</h3>
      <div className="h-80">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={sampleData}>
            ...
            />
            <Tooltip 
              formatter={(value: number) => [`${value.toFixed(1)}%`, 'Tariff Rate']}
      ...
      <p className="text-sm text-gray-600 mt-2">Click on bars to select countries for detailed analysis</p>
    </div>
  );
};

export const SectorImpactChart: React.FC<SectorAnalysisProps> = ({ sectorData }) => {
  ...
  const COLORS = chartData.map(item => IMPACT_COLORS[item.impact as keyof typeof IMPACT_COLORS] || '#6b7280');

  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <h3 className="text-xl font-bold text-gray-800 mb-4">Sector Impact Analysis</h3>
        ...
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              <Tooltip formatter={(value: number) => [`${value}%`, 'Tariff Rate']} />
            </PieChart>
          </ResponsiveContainer>
        </div>
            ...
            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center space-x-3">
                <div 
                  ...
                  style={{ backgroundColor: IMPACT_COLORS[sector.impact_level as keyof typeof IMPACT_COLORS] || '#6b7280' }}
                />
...
export const RegionalImpactMap: React.FC<RegionalImpactProps> = ({ countries, getCountryTariff }) => {
  ...
  const chartData = Object.entries(regionData).map(([region, countries]) => ({
    region,
    avg_tariff: countries.reduce((sum, c) => sum + c.tariff_rate, 0) / countries.length,
    country_count: countries.length,
  }));

  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <h3 className="text-xl font-bold text-gray-800 mb-4">Regional Impact Overview</h3>
      <div className="h-80">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData}>
            ...
            <YAxis 
              label={{ value: 'Average Tariff Rate (%)', angle: -90, position: 'insideLeft' }}
              fontSize={12}
            />
            <Tooltip 
...
export const TariffTrendChart: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <h3 className="text-xl font-bold text-gray-800 mb-4">Tariff Trend Analysis</h3>
      <div className="h-80">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={trendData}>
            ...
            <YAxis 
              yAxisId="right" 
              orientation="right"
              label={{ value: 'Affected Countries', angle: 90, position: 'insideRight' }}
              fontSize={12}
            />
            <Tooltip />
            <Line 
      ...
      <p className="text-sm text-gray-600 mt-2">Shows the progression of tariff rates and affected countries over time</p>
    </div>
  );
};

export const ImpactSummaryCards: React.FC<{ countryData: any }> = ({ countryData }) => {
  const getImpactLevel = (tariffRate: number) => {
    if (tariffRate >= 50) return { level: 'Critical', color: 'bg-red-600', text: 'text-red-600' };
    if (tariffRate >= 30) return { level: 'High', color: 'bg-orange-600', text: 'text-orange-600' };
    if (tariffRate >= 15) return { level: 'Medium', color: 'bg-amber-600', text: 'text-amber-600' };
    if (tariffRate >= 5) return { level: 'Low', color: 'bg-lime-600', text: 'text-lime-600' };
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-4 border-l-4 border-orange-500">
        <div className="flex items-center">
          ...
          <div className="ml-4">
            ...
            <p className={`text-2xl font-bold ${impact.text}`}>{impact.level}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-4 border-l-4 border-purple-500">
        <div className="flex items-center">
          ...
          <div className="ml-4">
            <p className="text-sm font-medium text-gray-600">Affected Sectors</p>
            <p className="text-2xl font-bold text-gray-900">{countryData?.affected_sectors?.length || 0}</p>
          </div>
        </div>
      </div>
...
const getRegion = (country: string): string => {
  const asiaCountries = ['China', 'Japan', 'South Korea', 'India', 'Thailand', 'Vietnam', 'Malaysia', 'Singapore', 'Indonesia', 'Philippines'];
  const europeCountries = ['European Union', 'Germany', 'France', 'Italy', 'Spain', 'Netherlands', 'Belgium', 'Sweden'];
  const americasCountries = ['Brazil', 'Mexico', 'Canada', 'Argentina', 'Chile', 'Peru', 'Colombia', 'Venezuela'];
  const africaCountries = ['South Africa', 'Nigeria', 'Kenya', 'Ethiopia', 'Ghana', 'Uganda'];
  const middleEastCountries = ['Saudi Arabia', 'UAE', 'Israel', 'Turkey', 'Iran', 'Qatar'];
