...
ENHANCED_COUNTRIES = {
    "Argentina": CountryData("Argentina", "ARG", 0.10, 0.10, "Americas", ["G20"], 487.2, 5.8),
    "Australia": CountryData("Australia", "AUS", 0.10, 0.10, "Oceania", ["G20", "Commonwealth"], 1550.0, 25.3),
    "Bangladesh": CountryData("Bangladesh", "BGD", 0.74, 0.37, "Asia", ["Commonwealth"], 460.2, 8.5),
    "Brazil": CountryData("Brazil", "BRA", 0.10, 0.10, "Americas", ["G20"], 2140.0, 35.2),
    "Cambodia": CountryData("Cambodia", "KHM", 0.97, 0.49, "Asia", ["ASEAN"], 27.2, 8.9),
    "Canada": CountryData("Canada", "CAN", 0.12, 0.10, "Americas", ["G7", "G20", "Commonwealth"], 2140.0, 429.8),
    ...
    "Costa Rica": CountryData("Costa Rica", "CRI", 0.17, 0.10, "Americas", [], 64.3, 12.5),
    "Dominican Republic": CountryData("Dominican Republic", "DOM", 0.10, 0.10, "Americas", [], 112.0, 10.7),
    "Ecuador": CountryData("Ecuador", "ECU", 0.12, 0.10, "Americas", [], 107.4, 2.3),
    "Egypt": CountryData("Egypt", "EGY", 0.10, 0.10, "Africa", [], 469.4, 3.1),
    "European Union": CountryData("European Union", "EU", 0.39, 0.20, "Europe", [], 17100.0, 515.3),
    ...
    "India": CountryData("India", "IND", 0.52, 0.26, "Asia", ["G20", "Commonwealth"], 3740.0, 119.4),
    "Indonesia": CountryData("Indonesia", "IDN", 0.64, 0.32, "Asia", ["G20", "ASEAN"], 1320.0, 30.1),
    "Israel": CountryData("Israel", "ISR", 0.33, 0.17, "Asia", [], 481.6, 14.7),
    "Japan": CountryData("Japan", "JPN", 0.46, 0.24, "Asia", ["G7", "G20"], 4940.0, 142.6),
    ...
    "Malaysia": CountryData("Malaysia", "MYS", 0.47, 0.24, "Asia", ["ASEAN", "Commonwealth"], 432.3, 40.9),
    "Myanmar": CountryData("Myanmar", "MMR", 0.88, 0.44, "Asia", ["ASEAN"], 76.1, 0.9),
    "New Zealand": CountryData("New Zealand", "NZL", 0.20, 0.10, "Oceania", ["Commonwealth"], 249.9, 4.2),
    ...
    "Pakistan": CountryData("Pakistan", "PAK", 0.58, 0.29, "Asia", ["Commonwealth"], 347.7, 5.4),
    "Peru": CountryData("Peru", "PER", 0.10, 0.10, "Americas", [], 242.6, 9.1),
    "Philippines": CountryData("Philippines", "PHL", 0.34, 0.17, "Asia", ["ASEAN"], 394.1, 18.9),
    "Saudi Arabia": CountryData("Saudi Arabia", "SAU", 0.10, 0.10, "Asia", ["G20"], 833.5, 16.2),
    "Singapore": CountryData("Singapore", "SGP", 0.10, 0.10, "Asia", ["ASEAN", "Commonwealth"], 397.0, 28.6),
    "South Africa": CountryData("South Africa", "ZAF", 0.60, 0.30, "Africa", ["G20", "Commonwealth"], 419.0, 8.9),
    "South Korea": CountryData("South Korea", "KOR", 0.50, 0.25, "Asia", ["G20"], 1810.0, 74.2),
    "Sri Lanka": CountryData("Sri Lanka", "LKA", 0.88, 0.44, "Asia", ["Commonwealth"], 80.7, 2.8),
    "Switzerland": CountryData("Switzerland", "CHE", 0.61, 0.31, "Europe", [], 812.9, 18.4),
    "Taiwan": CountryData("Taiwan", "TWN", 0.64, 0.32, "Asia", [], 669.0, 75.9),
    "Thailand": CountryData("Thailand", "THA", 0.72, 0.36, "Asia", ["ASEAN"], 534.8, 35.4),
    "Tunisia": CountryData("Tunisia", "TUN", 0.55, 0.28, "Africa", [], 46.8, 0.5),
    "Turkey": CountryData("Turkey", "TUR", 0.10, 0.10, "Europe/Asia", ["G20", "NATO"], 819.0, 8.4),
    "United Arab Emirates": CountryData("United Arab Emirates", "UAE", 0.10, 0.10, "Asia", [], 507.5, 13.8),
    "United Kingdom": CountryData("United Kingdom", "GBR", 0.10, 0.10, "Europe", ["G7", "G20", "Commonwealth"], 3130.0, 69.0),
    "Vietnam": CountryData("Vietnam", "VNM", 0.90, 0.46, "Asia", ["ASEAN"], 409.5, 77.3)
}

# Major Economic Sectors with US Import Dependencies
ECONOMIC_SECTORS = {
    "Technology & Electronics": SectorData("Technology & Electronics", 450.2, 0.85, 0.92),
...
def calculate_economic_impact_percentage(tariff_rate: float, trade_volume: float, gdp: float) -> float:
    """
...
def calculate_gdp_impact_usd(economic_disruption_pct: float, gdp_billions: float, years: int = 5) -> float:
    """
...
def get_impact_classification(economic_disruption_pct: float) -> EconomicImpactClassification:
    """Get authoritative impact classification"""
    for classification in IMPACT_CLASSIFICATIONS:
        if classification.threshold_min <= economic_disruption_pct < classification.threshold_max:
            return classification
    return IMPACT_CLASSIFICATIONS[-1]  # Severe impact for anything above highest threshold

def calculate_sector_impact_24_months(country: str, sector: str, tariff_rate: float) -> Dict:
    """
...
def get_countries_by_sort_option(sort_option: str) -> List[str]:
    ...
    if sort_option == "Alphabetical":
        ...
        group_priority = {"G7": 1, "G20": 2, "EU": 3, "ASEAN": 4, "Commonwealth": 5, "NATO": 6}
...
def format_country_display_name(country: str) -> str:
    ...
    groups = ", ".join(country_data.global_groups) if country_data.global_groups else "Independent"
...
def run_country_analysis(selected_countries: List[str], sort_option: str) -> Tuple[str, go.Figure, go.Figure]:
    """
    Tab 1: Comprehensive Country-Level Impact Analysis
    """
    if not selected_countries:
        return "Please select at least one country for analysis.", go.Figure(), go.Figure()
    ...
    for country in country_names:
        ...
        gdp_impact = calculate_gdp_impact_usd(economic_disruption, country_data.gdp_usd_billions)
    ...
    if results:
        ...
        fig1.add_bar(
            ...
            marker_color=['#1f77b4' if x < 5 else '#ff7f0e' if x < 10 else '#d62728' 
...
def run_sector_analysis(selected_countries: List[str], selected_sectors: List[str]) -> Tuple[str, go.Figure]:
    """
    Tab 2: Sector-Specific 24-Month Impact Analysis
    """
    if not selected_countries or not selected_sectors:
        return "Please select at least one country and one sector for analysis.", go.Figure()
    ...
    for sector_idx, sector in enumerate(selected_sectors):
        for country_idx, country in enumerate(country_names):
            ...
            country_data = ENHANCED_COUNTRIES[country]
            impact_data = calculate_sector_impact_24_months(country, sector, country_data.tariff_rate)
            ...
            if impact_data:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        legendgroup=country,
                        showlegend=(sector_idx == 0)  # Only show legend for first sector
                    ),
                    row=sector_idx + 1,
                    col=1
                )
...
def run_comparative_analysis(sector_for_comparison: str, countries_to_compare: List[str]) -> Tuple[str, go.Figure]:
    """
    Tab 3: Cross-Country Sector Comparison Analysis
    """
    if not sector_for_comparison or not countries_to_compare:
        return "Please select a sector and at least two countries for comparison.", go.Figure()
    
    # Extract country names
    country_names = [display_name.split(" (")[0] for display_name in countries_to_compare]
    
    if len(country_names) < 2:
        return "Please select at least two countries for meaningful comparison.", go.Figure()
    ...
    for idx, country in enumerate(country_names):
        ...
        country_data = ENHANCED_COUNTRIES[country]
        impact_data = calculate_sector_impact_24_months(country, sector_for_comparison, country_data.tariff_rate)
    ...
    if comparison_data:
        ...
        summary_lines.extend([
            f"### 🏆 Ranking by Peak Impact (Month 24)",
            ""
        ])
        ...
        with gr.Tabs():
            ...
            with gr.Tab("🌍 Country Impact Analysis"):
                ...
                country_selector = gr.CheckboxGroup(
                    choices=[format_country_display_name(country) for country in sorted(ENHANCED_COUNTRIES.keys())],
                ...
                analyze_btn = gr.Button("🔍 Run Country Analysis", variant="primary", size="lg")
                ...
                with gr.Row():
                    with gr.Column():
                        disruption_chart = gr.Plot(label="Economic Disruption by Country")
                    with gr.Column():
            ...
            with gr.Tab("🏭 Sector Analysis"):
                ...
                sector_analyze_btn = gr.Button("📈 Run Sector Analysis", variant="primary", size="lg")
            ...
            with gr.Tab("🔄 Cross-Country Comparison"):
                ...
                compare_btn = gr.Button("🔄 Run Comparative Analysis", variant="primary", size="lg")
                
                with gr.Row():
                    comparison_results = gr.Markdown(label="Comparative Analysis Results")
