...
st.set_page_config(
    page_title="TIPM: Tariff Impact Analysis", page_icon="📊", layout="wide"
)
...
def calculate_country_analysis(countries: List[str], sectors: List[str]) -> Dict:
    ...
    for country in countries:
        if country in SAMPLE_DATA:
            ...
            sector_impacts = [
                data.sector_impacts.get(sector, 0.0) for sector in sectors
            ]
            ...
            trade_dependency = min(
                data.trade_volume / 1000.0, 0.25
            )  # Max 25% dependency
...
def main():
    ...
    if preset == "Custom Selection":
        ...
        sectors = st.sidebar.multiselect(
            "Select Sectors:", all_sectors, default=default_sectors, max_selections=6
        )
    else:
