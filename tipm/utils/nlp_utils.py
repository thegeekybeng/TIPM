...
class PolicyTextProcessor:
    ...
    def calculate_sentiment_score(self, text: str) -> float:
        ...
        positive_words = [
            'benefit', 'growth', 'increase', 'improve', 'support',
            'enhance', 'strengthen', 'boost', 'advantage', 'opportunity'
        ]
        
        negative_words = [
            'tariff', 'penalty', 'restriction', 'ban', 'limit',
            'reduce', 'decrease', 'harm', 'damage', 'threat',
            'sanction', 'retaliation', 'dispute', 'conflict'
        ]
        
        neutral_words = [
            'policy', 'measure', 'regulation', 'standard', 'procedure',
            'implement', 'establish', 'maintain', 'review', 'monitor'
        ]
    ...
    def identify_policy_scope(self, text: str) -> Dict[str, Any]:
        ...
        primary_scope = max(scope_scores.items(), key=lambda x: x[1])[0] if any(scope_scores.values()) else 'unknown'
        ...
        return {
            ...
            'confidence': max(scope_scores.values()) / sum(scope_scores.values()) if sum(scope_scores.values()) > 0 else 0
        }
    
    def parse_policy_timeline(self, text: str) -> List[Dict[str, Any]]:
        """Parse timeline information from policy text"""
        timeline_patterns = [
            (r'effective\s+(?:from\s+)?(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', 'effective_date'),
