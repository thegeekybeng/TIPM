...
class TradeFlowLayer:
    ...
    def __init__(self, config: TradeFlowConfig):
        ...
        # ML models
        self.flow_predictor = RandomForestRegressor(
            n_estimators=100,
            random_state=42
        )
    ...
    def _initialize_default_network(self):
        ...
        self.country_to_index = {country: i for i, country in enumerate(major_countries)}
        self.index_to_country = {i: country for i, country in enumerate(major_countries)}
        ...
        default_routes = [
            ('CN', 'US', 500000), ('US', 'CN', 400000),
            ('CN', 'DE', 300000), ('DE', 'CN', 250000),
            ('US', 'DE', 200000), ('DE', 'US', 180000),
            ('CN', 'JP', 250000), ('JP', 'CN', 220000),
            ('SG', 'CN', 150000), ('CN', 'SG', 160000),
            ('SG', 'US', 100000), ('US', 'SG', 90000)
        ]
    ...
    def _build_trade_graph(self, trade_data: pd.DataFrame):
        ...
        route_aggregates = trade_data.groupby(['origin_country', 'destination_country']).agg({
            'trade_value': 'sum',
            'transport_cost': 'mean',
            'lead_time': 'mean'
        }).reset_index()
    ...
    def _prepare_training_data(self, trade_data: pd.DataFrame) -> Tuple[List[List[float]], List[Dict[str, float]]]:
    ...
    def _extract_route_features(self, origin: str, destination: str, hs_code: str) -> List[float]:
        ...
        origin_volume = self.trade_graph.nodes[origin].get('trade_volume', 0) if origin in self.trade_graph else 0
        dest_volume = self.trade_graph.nodes[destination].get('trade_volume', 0) if destination in self.trade_graph else 0
        
        features.extend([
            np.log1p(origin_volume),
            np.log1p(dest_volume)
        ])
    ...
    def predict(self, policy_features) -> TradeFlowPrediction:
        ...
        supply_chain_disruption = self._calculate_supply_chain_disruption(affected_routes)
        ...
        cost_increases = {route['route_id']: route.get('cost_change', 0) for route in affected_routes}
        time_delays = {route['route_id']: route.get('cost_change', 0) * 5 for route in affected_routes}  # Simplified
    ...
    def _identify_affected_routes(self, countries: List[str], hs_codes: List[str]) -> List[Dict[str, Any]]:
        ...
        for origin, destination, edge_data in self.trade_graph.edges(data=True):
            ...
            if origin in countries or destination in countries:
                route = {
                    ...
                    'hs_code': hs_codes[0] if hs_codes else '85',  # Default to electronics
    ...
    def _calculate_supply_chain_disruption(self, affected_routes: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    def _find_alternative_sources(self, affected_routes: List[Dict[str, Any]]) -> Dict[str, List[str]]:
        ...
        for route in affected_routes:
            ...
            for node in self.trade_graph.nodes():
                if (node != origin and 
                    self.trade_graph.has_edge(node, destination) and
                    node not in [origin]):
                    alternative_origins.append(node)
            
            alternatives[route['route_id']] = alternative_origins[:5]  # Top 5 alternatives
        
        return alternatives
    
    def _calculate_reallocation_matrix(self, affected_routes: List[Dict[str, Any]]) -> np.ndarray:
